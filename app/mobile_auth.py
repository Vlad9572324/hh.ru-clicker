...
class HHMobileClient:
    ...
    def create_browser_cookies(self, token: str, me: dict) -> dict[str, str]:
        ...
        """
        try:
            return self._create_browser_cookies(token, me)
        except Exception as exc:
            log_debug(f"mobile_auth create_browser_cookies error: {type(exc).__name__}: {exc}")
            raise

    def _create_browser_cookies(self, token: str, me: dict) -> dict[str, str]:
...
def upsert_browser_sessions(cookies: dict[str, str], me: dict, resumes: list[dict]) -> int:
    ...
    try:
        return _upsert_browser_sessions(cookies, me, resumes)
    except Exception as exc:
        log_debug(f"mobile_auth upsert_browser_sessions error: {type(exc).__name__}: {exc}")
        if isinstance(exc, MobileAuthError):
            raise
        raise MobileAuthError("Не удалось сохранить браузерную сессию", status_code=500) from exc


def _upsert_browser_sessions(cookies: dict[str, str], me: dict, resumes: list[dict]) -> int:
    """Implementation split out so every failure is logged at the public boundary."""
    if not cookies.get("hhtoken") or not cookies.get("_xsrf"):
    ...
    for resume in valid_resumes:
        ...
        changed += 1
    # Verify may only report success after the durable file has materialized.
    save_browser_sessions(sessions, wait=True)
    from app.instances import bot
    bot.reload_temp_sessions(sessions)
    return changed
